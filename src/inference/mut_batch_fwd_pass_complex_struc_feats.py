...
parser = argparse.ArgumentParser(description = """Batch fwd pass for predicting GPCR-peptide complexes with mutations introduced.""")
...
def init_features(msa_feature_dict, structure_feats, onehot_binder_seq, binder_length, config):
    ...
    #aatype
    new_feature_dict['int_seq'] = np.concatenate((np.argmax(msa_feature_dict['aatype'],axis=1), np.array(onehot_binder_seq)),axis=0)
    #between_segment_residues
    ...
    #residue_index
    new_feature_dict['residue_index'] = np.concatenate((structure_feats['A']['residue_index'],np.array(range(binder_length), dtype=np.int32)+max(structure_feats['A']['residue_index'])+201), axis=0)
    #seq_length
    ...
    #num_alignments
    new_feature_dict['num_alignments']=np.concatenate((msa_feature_dict['num_alignments'], msa_feature_dict['num_alignments'][:len(onehot_binder_seq)]), axis=0)
    ...
    #Structure feats
    new_feature_dict['atom14_gt_positions'] = np.concatenate((structure_feats['A']['atom14_gt_positions'], structure_feats['B']['atom14_gt_positions']), axis=0)
    new_feature_dict['atom14_gt_exists'] = np.concatenate((structure_feats['A']['atom14_gt_exists'], structure_feats['B']['atom14_gt_exists']), axis=0)
    new_feature_dict['atom14_atom_exists'] = np.concatenate((structure_feats['A']['atom14_atom_exists'], structure_feats['B']['atom14_atom_exists']), axis=0)
    new_feature_dict['chi_angles'] = np.concatenate((structure_feats['A']['chi_angles'], structure_feats['B']['chi_angles']))
    new_feature_dict['chi_mask'] = np.concatenate((structure_feats['A']['chi_mask'], structure_feats['B']['chi_mask']), axis=0)

    new_feature_dict['pseudo_beta'] = np.concatenate((structure_feats['A']['pseudo_beta'], structure_feats['B']['pseudo_beta']), axis=0)
    new_feature_dict['pseudo_beta_mask'] = np.concatenate((structure_feats['A']['pseudo_beta_mask'], structure_feats['B']['pseudo_beta_mask']))
    #Set the peptide positions to mod to get peptide FAPE and metrics
    new_feature_dict['mod_aa_index'] = np.zeros(len(['atom14_gt_positions']))
    new_feature_dict['mod_aa_index'][-binder_length:] = 1

    return new_feature_dict

def mutate_wt(int_binder_seq, n_muts, batch_size):
    """Mutate WT
    Introduce n_muts batch_size times
    using the standard 20 AAs (0-19)
    """
    ...
    for i in range(batch_size):
        mut_seq = copy.deepcopy(int_binder_seq)
        #First choose n_muts positions
        mut_pos = np.random.choice(range(binder_length), n_muts, replace=False)
        for mp in mut_pos:
            mut = np.random.choice(np.setdiff1d(range(20), mut_seq[mp]))
            #Update mut_seq
            mut_seq[mp] = mut
...
def eval_binder(config,
    ...
    #Mutate the binder seq randomly

    for n_muts in range(1, binder_length+1):

        #Check if prev pred exists
        if os.path.exists(outdir+predict_id+'_'+str(n_muts)+'_'+str(batch_size-1)+'.pdb'):
            continue
        mut_binder_seqs = mutate_wt(int_binder_seq, n_muts, batch_size)
        print('Making feats...')
        #Make feature dicts
        init_feature_dicts = [init_features(MSA_feats, structure_feats, mut_binder_seqs[x], binder_length, config) for x in range(batch_size)]
        batch = {}
        ...
        #Save all
        t0 = time.time()
        [save_structure(batch, prediction_result, i, predict_id+'_'+str(n_muts), outdir) for i in range(batch_size)]
