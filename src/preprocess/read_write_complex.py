...
def write_pdb(coords, seq, resnos, atoms, bfacs, occupancy, chains, outdir, pdbid):
    """Write PDB
    """
    ...
    with open(outname, 'w') as file:
        for i in range(len(chains)):
            ...
            for j in range(len(coords[chain])):
                atm_no+=1
                file.write(format_line(str(atm_no), atoms[chain][j], seq[chain][j], chain_letter, str(resnos[chain][j]), coords[chain][j],str(occupancy[chain][j]),str(bfacs[chain][j]), atoms[chain][j][0])+'\n')


    print('Saved complex to', outname)
##################MAIN#######################
